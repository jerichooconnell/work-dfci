...
def main():
    ...
    imgs2 = [s.pixel_data for s in xmf_list]
    headers2 = [s.header for s in xmf_list]

    np.save(f'projections/{sample_file_base.split(r"/")[-1]}_2nd_Layer', imgs2)
    np.save(
        f'projections/{sample_file_base.split(r"/")[-1]}_2nd_Layer_headers', headers2)

    reconstruct = False
    first_flood_file = ''
    if reconstuct:
        flood_first = np.load(first_flood_file)
        flood_second = np.load(second_flood_file)
        projs_1st = -np.log(imgs/flood_first)
        projs_2nd = -np.log(imgs2/flood_second)
        # load the angles
        angles = []
        angles2 = []
        for i in range(len(headers2)):
            # print(header_1st[i].keys())
            angles2.append(headers2[i][b'GantryRtn'][2])
            angles.append(headers[i][b'GantryRtn'][2])
        ...
        img1_exp = tigre.algorithms.fdk(
            projs_1st, geo, angles=-np.deg2rad(angles), verbose=True, filter='hamming')
        img2_exp = tigre.algorithms.fdk(
            projs_2nd, geo, angles=-np.deg2rad(angles2), verbose=True, filter='hamming')

    np.save(
        f'projections/{sample_file_base.split(r"/")[-1]}_1st_Layer_reconstructed', img1_exp)
    np.save(
        f'projections/{sample_file_base.split(r"/")[-1]}_2nd_Layer_reconstructed', img2_exp)
