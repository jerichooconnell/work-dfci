...
parser.add_argument('-d', '--device', required=False,
                    type=int, default=0, help="OpenCL device id")
parser.add_argument('-m', '--material', required=True,
                    type=str, help="Set a material name")
parser.add_argument('-p', '--process', required=True, type=str,
                    help="Set a physical process", choices=['Compton', 'Photoelectric', 'Rayleigh'])
parser.add_argument('-e', '--energy', required=True,
                    type=float, help="Set an energy in MeV")
...
print('    Photon energy cut (for 1 mm distance):',
      materials.get_energy_cut(material_name, 'gamma', 1.0, 'mm'), 'keV')
print('    Electron energy cut (for 1 mm distance):',
      materials.get_energy_cut(material_name, 'e-', 1.0, 'mm'), 'keV')
print('    Positron energy cut (for 1 mm distance):',
      materials.get_energy_cut(material_name, 'e+', 1.0, 'mm'), 'keV')
print('    Atomic number density:',
      materials.get_atomic_number_density(material_name), 'atoms.cm-3')

# -------------------------------------------------------------------------------
# STEP 5: Defining global parameters for cross-section building
processes_manager.set_cross_section_table_number_of_bins(
    220)  # Not exceed 2048 bins
...
print('    Attenuation: ', attenuations.get_mu(
    material_name, energy_MeV, 'MeV'), ' cm-1')
print('    Energy attenuation: ', attenuations.get_mu_en(
    material_name, energy_MeV, 'MeV'), ' cm-1')

print('At ', energy_MeV, ' MeV, cross section is ', cross_sections.get_cs(
    process_name, material_name, energy_MeV, 'MeV'), 'cm2.g-1')
