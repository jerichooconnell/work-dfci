...
results_df = pd.DataFrame(columns=['low_bin', 'high_bin',
                                   'fluence_W_to_bi_high',
                                   'fluence_W_to_bi_low',
                                   'fluence_bi_high',
                                   'fluence_bi_low',
                                   'mean_energy_W_to_bi_high',
                                   'mean_energy_W_to_bi_low',
                                   'mean_energy_bi_high',
                                   'mean_energy_bi_low',
                                   'mu_at_mean_energy_W_to_bi_low',
                                   'mu_at_mean_energy_W_to_bi_high',
                                   'mu_at_mean_energy_bi_low',
                                   'mu_at_mean_energy_bi_high',
                                   'std_mu_bi_low', 'std_mu_bi_high', 'cnr'])
...
for low_bin in range(1, 10):
    for high_bin in range(1, 10):
        ...
        fluence_W_to_bi_low, fluence_bi_low = calculate_fluence(
            yy_low, k_indices_low)
        ...
        U_high2, response_high_true = detector_response(
            yy_high_interp[k_bi_index_high:k_top_index_high2], xx_norm[k_bi_index_high:k_top_index_high2], c1)
        ...
        if DEBUG:
            ...
            plot_true_responses(U_high_bin, response_low_bin_low, response_low_bin_high, xx_low, U_low_bin,
                                response_high_bin_low, response_high_bin_high, xx_high, yy_low, c1, k_indices_low, k_indices_high, yy_high_interp)

        cnr = np.abs(mu_at_mean_energy_bi_high - mu_at_mean_energy_W_to_bi_low) / \
            np.sqrt(1/(std_mu_bi_high)**2 + 1/(std_mu_bi_low)**2)
