...
parser.add_argument('-v', '--vis', required=False,
                    action='store_false', help='Visualize the comparison')
...
def compare_images(image1_path, image2_path):
    ...
    image2 = 1.1*standard_scaler(image2)
...
def visualize_comparison(image1, image2, file_name1='GGEMS', file_name2='GATE'):
    ...
    plt.suptitle(
        f'{file_name1.split(".")[0].replace("_", " ")} vs {file_name2.split(".")[0].replace("_", " ")}')
    plt.tight_layout()
    plt.show(block=True)
    ...
    plt.suptitle(
        f'{file_name1.split(".")[0].replace("_", " ")} - {file_name2.split(".")[0].replace("_", " ")}')
    plt.tight_layout()
    plt.show(block=True)
    # Plot mean over Y
    plt.figure()
...
def standard_scaler(image, crop=20):
    return (image[crop:-crop, crop:-crop] - np.mean(image[crop:-crop, crop:-crop])) / np.std(image[crop:-crop, crop:-crop])


# Get all GGEMS and GATE MHD files
ggems_files = sorted(glob.glob('out/*fastcat*.npy'))
gate_files = sorted(glob.glob('out/*ggems_1e08*p.mhd'))
